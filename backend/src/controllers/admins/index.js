...
const createCandidateProfile = async (req, res, next) => {
    ...
    const schema = Joi.object({
      ...
      availability_start_time: Joi.string().pattern(REGEX.TIME_PATTERN).message('Invalid time format, must be hh:mm'),
      availability_end_time: Joi.string().pattern(REGEX.TIME_PATTERN).message('Invalid time format, must be hh:mm')
    })
